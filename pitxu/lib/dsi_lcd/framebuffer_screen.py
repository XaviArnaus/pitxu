...
class FramebufferScreen(PyXavi, Device):
    ...
    def _init_display(self):
        ...
        try:
            result = check_output("TERM=linux sudo setterm -cursor off -foreground black -clear all | tee /dev/tty1", shell=True).decode().strip()
    ...
    def close(self):
        ...
        try:
            result = check_output("TERM=linux sudo setterm -cursor on -foreground white -clear all | tee /dev/tty1", shell=True).decode().strip()
        except Exception as e:
